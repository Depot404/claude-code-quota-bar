...
- **Every conversation in the current workspace, with live state** — working / waiting for you / done / stale — not just the tab dot VS Code's own extension shows (a blue dot for a pending permission, an orange one for a finished hidden tab — nothing at all for a conversation asking you a question). A finished conversation keeps a **bright ✓ until you click its row** ([read receipts](#read-receipts))
...
> Talks to an Anthropic endpoint that **isn't part of the documented public API**, and runs on Windows, macOS or Linux — two extras degrade cleanly off their home platform (window-raising is Windows-only, sounds work on Windows and macOS but stay silent on Linux) — see [Requirements](#requirements) and [Known limitations](#known-limitations) before installing.
...
![The New conversation form after pasting a claude-convs block: three prompts prefilled across two waves, each with its own model and effort selectors, and a "Create 3" button](images/screenshot-new-conversation.png)
...
- **Several prompts = a batch.** Click **+ Add task**, or simply **paste a `claude-convs` block into any prompt field** — the form recognizes it and prefills everything: one task per section, model, effort, waves — and the batch takes the block's own `group:` name, with no field to fill in. Type **`/handoffs`** in any Claude conversation to have it end its reply with exactly that block, ready to paste: plan in one conversation, paste once, launch everything.
...
- **A click on any row can set — or remove — the master conversation, before you create.** While a block is being composed, every row outside a batch — a plain row, or a batch's own head row — carries a ribbon: *set as the master conversation* if it isn't the master yet, *detach from this conversation* if it is. The choice stays local to the form until Create, which then carries it verbatim — a chosen master is adopted without re-running the lookup, and choosing "none" starts the work on its own without ever searching the pasted text: a batch when the block holds several tasks, a single ordinary conversation when it holds one. Either way the preview stays on screen, under the **New conversation** header, so you always see what Create is about to open. Rows already inside a batch (other than its head row) keep their own sibling/nesting rules — see below — the ribbon never appears there.
...
- **Pasting a new block from a conversation that already heads a batch continues that batch.** Create adds the tasks after its last wave instead of opening a competing group that would take the header away from it — they queue, and the wave engine opens them in turn.
- **Pasting from a conversation that is a plain member of another live batch joins that batch as a sibling, by default.** The new tasks queue in the same batch, right after her turn — landing on the later of her own wave + 1 and the batch's currently running wave + 1, so they never try to jump ahead of a wave already in flight. Hovering any other row of that same batch works as an ordinary insertion target (into/before it), exactly as for a block with no master; hovering any other batch is refused. Nesting the new tasks *under* her row instead is still available — it now takes an explicit gesture, see [Nested batches](#nested-batches) below.
- **Pasting from a conversation that heads nothing — outside every batch — founds a new batch, as before.**
- **Each member row is a live conversation row** — same states, same `model · effort` badge, same context bar as the flat list, down to the pixel: it *is* the same row rendering, and nothing in a group is allowed to shorten it. A task whose conversation hasn't opened yet shows a waiting ring on the group's rail. Closing a member's tab removes its row here exactly as it does in the flat list — one row is one tab, in both views — while the header's "x/y done" tally stays right: it counts finishes recorded in the batch itself, not the rows currently on screen.
...
It's common to paste a `claude-convs` block *inside* a conversation that already belongs to another batch — the plan for wave 2 is often written from within wave 1's own conversation. By default those new tasks join as siblings of that conversation, in its own batch (see above) — nesting them *under* her row instead is a deliberate gesture, not the default: hover the master's own row — its ribbon reads *nested under this conversation* — and click it, or click **Create** while hovering it. Either way, the new batch then nests under the row that opened it instead of showing up as a separate group:
...
- `~/.claude/active-session.json` — which conversation received the last prompt (model display, and the `/handoffs` token). It no longer decides the highlight: the highlighted row follows the currently selected tab, per window, and nothing else.
...
| `done` | **bright ✓** / dim ✓ | Finished replying (`Stop`). Bright until you click its row — see [Read receipts](#read-receipts) |
...
A conversation is only listed while its tab is open somewhere — closing the tab removes the row, whether or not the CLI process behind it is still running (2.60.0). A conversation pinned "to review" is the one exception. See [When a conversation disappears](#when-a-conversation-disappears).
...
The ✓ of a finished conversation stays **bright until you click its row in the panel**, and only then dims. It never expires on a timer, and nothing else can dim it.

- **The click is the whole rule** (since 2.27.12), and it is taken unconditionally — even when the tab is already active, the one case where no tab switch can ever happen. Clicking a row also focuses that conversation's tab, so reading it and marking it read are the same gesture.
- **Nothing is inferred from where you are looking any more.** The old rule — the matching tab *active*, the window focused, held for ~2 s — never meant *read*. It fired for a tab left open while you worked elsewhere in the same window, for the official extension's own end-of-turn `rename_tab` (which looked like a fresh arrival, dimming the ✓ a constant ~2.3 s after every `Stop`), for tabs opened by the wave launcher, and finally for a window simply coming back to the foreground with that conversation already active. Four guards had been stacked on that path over as many releases; a signal needing five exceptions was not measuring what it claimed to. Before all of that, the ✓ simply faded after 30 min — an arbitrary delay that erased results you never read and kept bright ones you'd read half an hour ago.
- **The dwell tracker still runs, and only writes to the journal.** `ack.js` observes tab visits exactly as before; it decides nothing.
...
- **Diagnostic log** (2.27.7, `ack-journal.js`). This path depends on VS Code tab events, a process registry and timestamps that cannot be replayed after the fact, so what happens is written as it happens: one JSON line per read receipt posted and per notable verdict, with the full context of that decision, appended to `~/.claude/quotabar-ack-journal.jsonl` (local only, rotated past 1 MB, one generation kept). Highlight, click-to-focus and presence verdicts share the same file. It records; it never decides — no verdict depends on it, and a failed write is silently ignored. Set `QUOTABAR_ACK_JOURNAL=off` in the environment to disable it, or point it at another path.
...
The title in the transcript (`ai-title`) is **not** always the tab caption. The official extension re-labels a reopened or restored tab with the conversation's **last prompt** (24 characters + `…`), and keeps that caption even after new messages are sent. Matching on `ai-title` alone then finds nothing: the conversation looks tab-less and gets hidden, and clicking its row focuses nothing. So every place where a tab caption meets a conversation tries both texts — the transcript title and the last prompt.

Identity, not text, is the primary source: the workspace's `state.vscdb` holds the editor memento (`workbench.parts.editor`), which names the session behind each Claude tab of this window and gives its position. That is what click-to-focus, row highlighting and presence use first; captions are only the fallback. This memento and the live-session registry are undocumented internals, read **read-only** (reopened and closed on each refresh, at most once per 30 s tick): if either is missing or unreadable, the panel silently falls back to its previous behaviour, and neither can hide a conversation that would otherwise show.
...
Every tab↔conversation match — click-to-focus, the presence filter above, the highlight — depends on `viewType.includes('claudeVSCodePanel')` (`labels.js`). If the official Claude Code extension ever renames that `viewType`, none of these paths raise an error: they just silently stop matching any tab, and the panel quietly degrades.
...
### When the highlight is filled in

VS Code's extension host only ever sees tabs through a mirror synced from the renderer over RPC. That mirror gives a tab **label**, never an identity — and a Claude tab can be renamed to the first words of the prompt it was given, so its label matches no row in the panel. When that happens the panel would light up nothing at all.

So it keeps a second source, consulted only when the label decides nothing: the editor layout VS Code's **renderer** persists to workspace storage — the process that actually paints your screen, recorded by exact session identity rather than by tab label. If that record is more recent than the tab tracker's last change of mind, it names the row to highlight. It only ever *fills a blank*: any row already picked by a label or by a click stays picked, and the record never overrides it.

> Versions before 2.110.0 also let that record *correct* an existing highlight, with a visible banner. That existed for a VS Code bug ([microsoft/vscode#331914](https://github.com/microsoft/vscode/issues/331914)) where a background tab could be reported as the active one — fixed upstream in VS Code 1.135, which QuotaSaver now requires. The whole mechanism, and the frozen-mirror notice that went with it, are gone.
...
- [Claude Code](https://docs.anthropic.com/claude/docs/claude-code) installed and signed in — the quota bars read its OAuth token and work out of the box.
- VS Code 1.135 or later (the Secondary Side Bar panel needs 1.106; 1.135 is required for correct tab reporting — see *When the highlight is filled in*).
- **Windows, macOS or Linux.** The panel, conversation state, quota bars and batching are plain Node — nothing OS-specific. Two optional extras lean on Windows and degrade cleanly elsewhere, never with a visible error: window-raising (Windows only — no portable equivalent, see [Clicking a conversation](#clicking-a-conversation)) and sounds (Windows and macOS, silent on Linux, see [Sounds](#sounds)).
- **Everything above works with zero configuration** — conversations, `ctx:%`, and the quota bars. One thing is opt-in and degrades cleanly when skipped, never with an error: **live conversation state** (`busy`/`waiting`/`done` instead of `idle`), which needs the hooks — see [Setup](#setup).
...
1. **Find the tab by identity, not by text.** The renderer records, in workspace storage, which session each open editor holds and *where* it sits (editor group + index). That record is validated as a whole — the same photo, judged by the same code, for the click and for the highlight, so the two can never disagree — and the tab at that position is selected. Two conversations can carry a strictly identical truncated tab label, which no comparison of text can ever tell apart; a position tied to a session id can. The only text check left is a *positive* one: the attempt is abandoned if the label at that position names **another** listed conversation (tabs reordered since the record was last written). A label matching nothing in the list is not evidence against an identity — a restored tab is often re-titled from the last prompt it received.
2. **Fall back to the label, and search every editor group.** When no identity is available (the record unreadable, or the session missing from it — the usual case in the first seconds after a tab is reopened), the tab is matched by label: the Claude Code extension truncates it to 24 characters plus an ellipsis (`Refactor auth middlewar…`) while the panel shows the full title, so the label is compared as a *prefix* of the title when truncated, exactly otherwise. A tab can carry **two** names: the conversation's generated title, and — on any reopened or restored tab — its **last prompt**, which Claude Code puts there and keeps; both are matched. Nothing matches → nothing happens, rather than focusing the wrong conversation. **Several open tabs match → nothing happens either**, and the row's ≈ badge lights up to say why: two tabs carrying one name cannot be told apart by text, so the panel refuses to guess (click the tab itself, or wait for the position record to be written). Either way the tab is activated with `workbench.action.openEditorAtIndex` — a command that can only ever *choose* among existing tabs, never create one — which acts on the active group only, so the group is focused first (`workbench.action.focusNthEditorGroup`).
...
- **QuotaSaver: Install Hooks** — deploys the hooks (see [Setup](#setup)) after showing exactly what will be written, for live conversation state.
- **QuotaSaver: Add Batching Philosophy to CLAUDE.md** — optional, see [Batching philosophy](#batching-philosophy-optional); shows the exact text before adding it.
...
- A read receipt is written only when you click a row; what's persisted is a pair of timestamps (`ack_ts`, `busy_since`), on your machine. The tab observer that remains (journal only) sees **only** which tab is active and whether the window has focus — never what's in it.
...
- **No API gives an editor tab's Claude session**, none activates a tab ([microsoft/vscode#158853](https://github.com/microsoft/vscode/issues/158853), [#162446](https://github.com/microsoft/vscode/issues/162446)), and none raises a window ([#51078](https://github.com/microsoft/vscode/issues/51078)); the Claude Code extension contributes no usable command targeting a `session_id` either — the one that exists *opens* a tab when the session's webview is asleep, which after a window reload is the common case. The panel therefore reads the renderer's own record of which editor holds which session (see [Clicking a conversation](#clicking-a-conversation)). What that can't do: a conversation whose tab is closed everywhere is a no-op, and while that record is still being written — the first seconds after tabs are reopened — two tabs **carrying the same name** cannot be told apart, so the click is refused rather than guessed (the ≈ badge on the row says so). Selecting the tab yourself always works, and the refusal lifts on its own once the record catches up. No tab is ever created or closed in the meantime.
- The panel needs the Secondary Side Bar view container contribution (**VS Code 1.106**), and correct `activeTab` reporting ([microsoft/vscode#331914](https://github.com/microsoft/vscode/issues/331914), fixed in **1.135**) — hence `engines.vscode: ^1.135.0` in `package.json`.
- **A conversation whose title is a fallback (no generated title yet — typically a brand-new conversation) can't be matched against a tab label**, so "no match" proves nothing about it and it stays listed on that basis alone. Such a row leaves the list on an observed event or on time: an explicit tab close seen while the extension was running, or 4 h of inactivity. (Until 2.114.0 a second source settled it — a tab-title table in the workspace database — but VS Code no longer fills it in: measured at 2 entries for 7 open tabs, so it was removed rather than trusted.)
...
- **A reply read on its own tab, without ever clicking the panel row, stays bright** (deliberate, see [Read receipts](#read-receipts)): a click is the only thing that dims a ✓. Every attempt at inferring "you have read it" from tab activity produced false receipts, which are far worse than a ✓ that stays lit a while longer.
