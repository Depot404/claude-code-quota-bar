...
### Changed
- **QuotaSaver now requires VS Code 1.135 or later, and the seven workarounds it needed before that are gone.** For three weeks, one symptom kept coming back: the panel highlighted a conversation you were not looking at. Every fix targeted a mechanism that had been *guessed*. The real cause was in VS Code itself — a background tab could be reported as the active one after any title or icon change ([microsoft/vscode#331914](https://github.com/microsoft/vscode/issues/331914)) — and it was fixed upstream in **1.135.0**. So the workarounds are removed rather than kept alive: the held-highlight rule, the quarantine and revocation tied to conversation state changes, the frozen-mirror detection with its click-wins-over-the-API rule, the unfocused-window grace period, and the renderer judge's ability to *correct* an existing highlight (with its banner and its 45-second margin). On 1.135 and later, what the tab mirror reports is simply true, and the highlight follows the selected tab with no filtering in between.
- **What still guards the highlight, unchanged:** tab positions matched by session identity rather than by label; the label check that only rejects an identity when it names *another* listed conversation; the tolerance that keeps a row on screen through an ambiguous miss; and the renderer record, which still *fills in* a highlight the tab label alone cannot resolve — the case of a tab renamed to the first words of its prompt. It never overrides a row already picked.
- **`~/.claude/active-session.json` no longer decides which row is highlighted.** It records which conversation received the last prompt — which is not the same question as which tab is on screen: you can submit in one conversation and go read another, and the highlight had no way back from that. Two places used it as a tiebreaker (two conversations sharing a truncated tab label, and a window with no Claude tab selected); both now stay silent instead, which lets the renderer record name the conversation actually displayed. The file itself, and everything else that reads it, are unchanged.
- **One capability is lost, and it is worth naming:** if a window's tab channel ever stops reporting entirely, nothing detects it any more, and the `VS Code stopped reporting tab changes` notice is gone with it. The symptom is a highlight that stops following your tabs; reloading the window fixes it.

## [2.109.0] - 2026-09-03

### Fixed
- **A conversation whose tab was renamed by Claude Code is clickable again, and its row lights up at once.** After a window reload, Claude Code rebuilds a restored tab's title from the last prompt in memory (a tab reading "ok go" for a conversation the panel still calls by its real title) — a fact read in its source, not guessed. The panel already knew *which* tab belonged to that conversation, by identity and at the right position, but then vetoed its own answer because the label at that position did not match the title: the click did nothing, and the row stayed dark until a slow secondary check filled it in seconds later, only to go dark again at the next tab switch. A label that matches *no* conversation is not evidence against an identity; a label that names *another* listed conversation is (tabs reordered since the last snapshot). That is the check now — same rule for the click and for the highlight, applied at the same instant. The diagnostic entries added in 2.108.0 made this a one-line read: `focus-identity outcome:label-mismatch` on a correct position.

## [2.108.0] - 2026-09-03

### Changed
- **Clicking a conversation whose tab focus lands on the wrong one now leaves a trace to diagnose from.** A small number of clicks were observed landing on an unrelated conversation's tab instead of the one clicked, with no way to tell why after the fact. Every focus attempt now logs which path resolved it (exact position, or a text-label fallback) and what every open conversation tab looked like at that instant, in the same local, private log already used for other diagnostics. Nothing about how a click is resolved changes; only what gets recorded about it.

### Fixed
- **The preview no longer announces a wave one further along than where the tasks actually land.** Hovering a row of a live batch with a single-wave block in the form said "wave 3 — queued" while the store filed the task into wave 2 — the very wave the ribbon underneath promised to join. Two different meanings were riding on the same number: *which wave the tasks will belong to* and *which existing wave the preview must be drawn in front of*. Those are the same thing when a multi-wave block pushes a wave aside, and differ by exactly one when a single-wave block joins an existing wave — which is why only single-wave blocks were affected. They are now carried separately, and the number shown is the one sent to the store, nothing else. Same fix, one consequence worth stating: a block joining the wave that is **currently running** is no longer labelled "queued", since it starts the moment you click.
- **A block pasted by a conversation that already heads a live batch now previews the wave numbers it will really get.** Create does not open a second batch in that case — it appends the tasks after the last wave of the batch that conversation already leads — but the preview kept numbering them from 1, so a batch already on its second wave showed "wave 1" for tasks that were about to become wave 3. The panel now reads the batch to be chained exactly as the extension does when it decides to chain, so both answer the same question with the same numbers. Where the preview sits on screen is unchanged: at the bottom of that batch's body, where the tasks will really appear.
- **Designating a master conversation and clicking Create without moving the mouse no longer leaves the ribbon and its frame behind.** The "⌂ set as the master conversation" ribbon was only ever taken down when the pointer left the row, so a click on Create straight afterwards left it hanging over a row that is no longer a target, with the form already empty. Taking the decoration down is now the job of the same code that draws it — which runs on every change to the form (typing, ×, Cancel, Create, incoming state) — for both kinds of hover decoration at once, rather than of a single mouse event.

## [2.106.0] - 2026-09-02

### Fixed
- **The row of the tab you are looking at lights up again, even when that tab has been renamed.** On VS Code 1.135 and later, nothing was highlighted at all for conversations whose editor tab no longer carries their title — measured that day: two tabs went back to showing the beginning of their task's prompt while the panel kept the conversation title, so no row matched the active tab and the panel had nothing to light. The cause was ours, not VS Code's: 2.100.0 switched off the defences that a trustworthy tab mirror makes pointless, and took with them the one source that supplies an *identity* — the editor identity written to disk by the renderer. A trustworthy mirror gives the right *label*, never an identity, so when that label matches no row there was simply nothing left. That source is now consulted again on every VS Code version, but only to **fill an empty highlight**: it still cannot override a row the panel already chose, which remains reserved for versions whose mirror can lie. Filling an empty highlight overrides nothing and raises no warning banner, as before.

### Fixed
- **A single task launched with a `group:` name now shows up on screen again.** 2.104.0 made a solo task stop founding a batch on a group name alone, on the grounds that the name is never displayed anywhere. That was true, but it broke a stronger guarantee: before the first Enter in the launched tab, the transcript does not exist yet, so the flat list — which only lists conversations that have both a transcript and a tab — has nothing to show either. The batch was the *only* thing carrying that task's state, so removing it left "Create" open a VS Code tab with the prompt inserted and nothing at all in the panel. A single task with a `group:` name founds a batch again, exactly as before 2.104.0.
- **A solo batch (one task, no master) now shows a much smaller grip instead of disappearing.** The actual complaint behind the change above was real — a full batch header (chevron, manual/auto switch, "0/1 done" counter, ⌂) around a single conversation is chrome for information nobody can see. Rather than refuse the batch outright, only the parts that make no sense for a single member are now hidden: chevron, the manual/auto switch and the counter. What is left is the batch's identity (still shown, so it stays a batch you can tell apart from a plain conversation) and the dissolve button, revealed on hover as usual. Clicking the reduced header no longer collapses anything — there is no chevron promising it would. As soon as a master conversation is resolved for that batch, the full grip comes back.

## [2.104.0] - 2026-09-02

### Fixed
- **A pasted block with no master conversation keeps its preview.** Detaching the master with a click — or a lookup that finds nothing — used to make the whole preview of the future conversations vanish, leaving the form with nothing to show for what Create was about to open. The preview no longer depends on there being a master's row to hang under: with none, it sits at the top of the **New conversation** body, just below the section header, and follows that section when you collapse it. No staple is drawn in that case, and that is deliberate — the staple states a lineage, and there is none left.
- **Detaching the master no longer reads as a failed lookup.** A deliberate click showed the same warning as a search that found nothing (*⚠ No master conversation found*). It now says what happened — *⤴ Master conversation detached · standalone batch* — while a real lookup failure keeps its warning, unchanged.
- **A single task no longer gets a whole batch header just because the block carries a `group:` name.** Pasting a one-task `/handoffs` block wrapped that lone conversation in a full batch grip — chevron, manual/auto switch, "0/1 done", ⌂, ✕ and a rail — for a name that is displayed nowhere (the grip shows the creation time, and the name only ever served as an internal fallback). A single task now founds a batch only when a master conversation is resolved, since the master has no other surface than the batch's head row. Several tasks still always make a batch.

## [2.103.0] - 2026-09-02

### Fixed
- **After reloading the window, conversations whose tabs you have not visited yet no longer vanish from the panel.** Five Claude tabs open, two rows listed — some coming back later, others never (reported with a screenshot). VS Code only deserializes a restored webview the first time you visit it: until then the tab is literally titled "Claude Code", so no conversation matches it, and the panel read that as proof the tab had been closed. The generic title is now treated as what it is — a tab whose content is not known yet, never a tab known to hold something else. While any such tab is around, presence is read from the editor identity the renderer wrote to disk (which survives a reload) *in addition to* tab titles, so a conversation is kept if either source vouches for it. Measured over four reloads the same day, the real titles came back 3 to 10 minutes later, each one exactly when the tab was visited — which is why the previous 90-second grace period could not cover this: it was counting time, while what mattered was a gesture. A genuinely closed conversation still disappears at once, and with no editor identity available (locked or unreadable database) behaviour is unchanged.

- **A conversation older than four hours whose restored tab you have not visited yet is no longer dropped for age either.** The age filter runs before anything else and only knew tab titles, so it removed such a conversation before presence had any say — fixing presence alone would never have brought it back. It now accepts the same editor identity as the rest, so "a tab is open ⇒ the conversation is listed" holds again on both gates that enforce it before a single transcript is read. An old conversation with no tab at all is still dropped, exactly as before.

### Added
- **The journal now records why a conversation left the list.** Until now only highlight decisions were journalled; when rows went missing there was no line saying which rule had dropped them, and the diagnosis above had to be reconstructed from highlight verdicts, which only ever describe the active tab. A `presence-drop` entry is written whenever the composition of the list changes — how many rows are shown, how many tabs are still unresolved, and for each dropped conversation the evidence that was available about it.

## [2.102.0] - 2026-09-02

### Changed
- **A click on any row now designates — or removes — the master conversation of the block being composed.** A pasted block with no `session:` line still attached itself to whichever conversation's transcript matched its text, with no way to refuse that link before Create. Any row outside a batch — a plain row, or a batch's own head row — now carries a ribbon while a block is being composed: "set as the master conversation" if it isn't the master yet, "detach from this conversation" if it is. The choice stays local until Create, which then carries it verbatim: a chosen conversation is adopted without re-running the search, and an explicit "none" founds an autonomous batch without ever searching the pasted text. Rows inside a batch keep their existing sibling/nesting/refusal rules, untouched — the ribbon never appears there.

### Fixed
- **A pasted block whose master conversation has already finished a wave behind the batch's current one no longer drops silently.** The default sibling place (2.101.0) always targeted the master's own wave + 1 — safe only as long as that wave was still queued. A master who had finished wave 1 while her batch had already moved on to wave 2 made the target land on the very wave that was already running: the store refuses that (a queued wave can't jump ahead of a running one), and neither the preview nor a Create click ever said so — the tasks simply never appeared. The target is now the later of the master's wave + 1 and the batch's running wave + 1: always just after her turn, never in the past.
- **"Relaunch" is now offered on a task whose wave opened but never got any session id attached at all** — not only, as until now, one whose linked process died before sending. Nothing having started under any identifier makes the case for relaunching even stronger than the one already covered for a dead link (PLAN_lien_mort_ne_2026-08-04.md), and without this the wave stayed open forever: "Link…" was never the gesture used in practice on this case. A one-line confirmation is asked first — a tab may still be sitting open with the prompt inserted, waiting for Enter.
- **A queued task's tooltip now shows its prompt, in full.** It used to carry only the state explanation ("queued", "waiting for you", "link lost before sending"…) — never the prompt itself, which stays truncated on screen by CSS, so a stuck task's prompt could not be read or copied. The prompt now comes first, the state explanation right below it.

## [2.101.0] - 2026-09-02

### Changed
- **A pasted block whose master conversation is already a member of a live batch now joins that batch as a sibling by default, instead of silently nesting under it.** A `/handoffs` block pasted with `session:` used to always found a nested sub-batch under its master's row — correct when the master started the batch, wrong when she is herself a rank-and-file member of an existing one: the new tasks belonged right after her turn, in the same batch, not in a batch of their own. Hovering any other row of her own batch is now an ordinary insertion target, with the same into/before rules as a block with no master; hovering any other batch is still refused, with its ribbon reworded ("the master conversation sets the batch") since a default place already exists elsewhere. Nesting is still available — it now takes hovering the master's own row explicitly ("nested under this conversation"), and only that gesture, or Create while hovering it, still founds a separate batch. A master that isn't in any live batch, or is already the head of one, behaves exactly as before.

### Changed
- **On VS Code 1.135 and later, the panel takes the tab API at its word — every "phantom tab" defence is switched off.** Three weeks of highlight incidents (a background conversation stealing the highlight the moment it finished, a click on a panel row that "did not take", an unfocused window whose highlight drifted for hours) had a single cause, and it was not in this extension: VS Code never cleared the `isActive` flag of the tab you had just left, and re-sent that stale cached tab to extensions whenever its title or icon changed — so any tab you had visited once declared itself active every time it was rewritten (microsoft/vscode#331914, fixed upstream on 2026-08-21, shipped in 1.135.0). On 1.135+ the mirror tells the truth, so an activation is a gesture, focused window or not; nothing is held, quarantined or revoked; no frozen channel is ever detected; and the disk-based judge that corrected the highlight after the fact — with its "highlight corrected" banner — is inactive. Older VS Code versions keep every defence exactly as before: the switch is the version of the VS Code that hosts the extension, read once at startup.

### Fixed
- **The "master conversation sets the place" ribbon no longer outlives the block it was refusing.** Removing the last task with the form's × (or cancelling) left the ribbon, the lit wave and the highlighted prompt field on screen: only moving the pointer off the row cleared them. The target is now dropped the moment the form has no task left, on the same rendering pass that removes the preview.
- **While a block is being composed, the ⤴ "leave the batch" button of every row is gone.** Rows are drop targets during composition, and that button lives on the same pixels: on a refused target the click went straight through to it, and the host conversation just above the nested preview could be pulled out of its batch by what looked like a click on the preview (reported with a screenshot on 2026-09-02). The button comes back as soon as the form is empty.

## [2.99.0] - 2026-09-02

### Fixed
- **A launched task can no longer claim a conversation it did not open.** The panel showed *⚠ asked for sonnet · medium* on a batch's master conversation, which no task had ever opened — the prompt had gone to a different one. When a batch opens a tab, the CLI writes a session file moments later, and the launcher took the first session id that was not in the photo taken just before: an arrival order, not proof. A session that is minutes or hours old looks brand new when its file happened to be unreadable at photo time (it is being rewritten, so the entry is skipped) and readable again during the ten seconds that follow; a conversation you start by hand in that window is just as claimable. The launcher now reads the session's own start timestamp and adopts only a process that started *after* the call it just made — one that started before cannot be the tab being opened. Two provably new candidates at once means no link at all, never a coin toss, and a CLI too old to write that timestamp leaves the task unlinked for the prompt-matching stage to pick up, exactly as an unseen session already did.
- **A refused link no longer leaves its intention behind.** The batch store already refuses to attach a group's master conversation as one of its own members — that refusal is what kept the mislink out of storage. But the intention (the model and effort that had been asked for) was recorded *before* the attempt, so the refusal undid nothing and the badge stuck to the master anyway. Launch, wave opening and *Relaunch* now record the intention only once the store has accepted the link, as the two automatic re-linking stages already did.
- **Mislinks written by earlier versions repair themselves at startup.** A member whose linked session started before its own launch is proof of a past mislink: the link is dropped, so the phantom badge goes without any manual gesture. The task itself is untouched — prompt, model, effort and wave stay, so *Relaunch* and automatic re-linking still work. The doubt always favours what is already there: a session missing from the registry (its process is gone) or one that started later (a reload respawned its CLI) is kept as is.

## [2.98.0] - 2026-09-01

### Fixed
- **A pasted block with a master conversation can no longer be dropped into the wrong batch.** While such a block sat in the form, hovering the rows of an unrelated batch moved the preview there, as if you could pick the spot. You could not: clicking would have poured the tasks into *that* batch through the "add tasks" path, which carries neither the pasted text nor the `session:` token — the master link was dropped in silence, and the batch you got was not the one the preview had shown. Every row target is now refused for as long as the form carries a master, through the same refusal the panel already uses for a wave it cannot reach: same ribbon, no insertion frame, inert click, no renumbering. The ribbon says why — *the master conversation sets the place*. A form without a master is untouched: hovering and clicking still choose the spot.
- **The preview now shows the nesting the batch will actually have.** When the master conversation is a member of an existing batch, the new batch renders *nested* under her row — indented, on its own rail. The preview drew flat siblings in the flow instead, so the result never looked like what had been announced (reported with screenshots on 2026-09-01). It now takes the shape and the place of the sub-batch body it will become, closing hook included. The flat choice made on 2026-08-29 stands for a top-level master, which was the case it was decided on.

## [2.97.0] - 2026-08-31

### Fixed
- **After a window reload, an open conversation no longer shows its title struck through for half a minute.** Since 2.86.2 the row's *presence* is judged, during the 90 s that follow a reload, on the renderer's editor memento — the one source that survives the reload. But the field that draws the strikethrough (`tabOpen`) was still judged on tab-label pairing, and right after a reload the respawning CLIs have not republished their labels yet: a finished conversation, whose process was not alive yet either, burned through its two-miss tolerance in the reload's burst of recomputes and got struck as "finished, tab closed" while its tab sat wide open on screen (reported on the 2.96.0 reload). During the grace, `tabOpen` now follows the same memento as presence — one display fact, one source. Bench: test-presence.js §14, which now runs two recomputes with the engine's shared tolerance map, red before the fix.

## [2.96.0] - 2026-08-31

### Fixed
- **A window you are not using can no longer have its highlight stolen while you are away.** VS Code's tab mirror sometimes fires a burst of `changed` events, each claiming a different tab just became active, in a window that has had no focus for hours — the screen never moved. The panel adopted the last claim, and the disk-based judge that normally corrects such lies within a minute was disqualified forever: VS Code only rewrites its editor memento while you interact with the window, so the judge's clock had stopped two hours before the lie. Measured on 2026-08-31: highlight wrong for 5½ hours, still wrong when the user came back. The rule that closes the class: an unfocused window receives no human gesture on its tabs, so a `changed` activation there is never proof. The one real exception — the click that both focuses the window and activates a tab, whose tab event may arrive milliseconds before the focus event — is kept: such a flip is adopted when focus follows within 3 s, and expires otherwise. Structural changes (a tab opened active by a wave launch, the active tab closing) remain full proof, focused or not. New bench `test-unfocused-phantom.js` replays the incident on the real tab tracker — the previous 2000-gesture loop stubbed it, which is why it never saw this.

## [2.95.0] - 2026-08-31

### Fixed
- **The "asked for X" warning is back, and it stays for as long as the conversation lives.** Since 2.78.2 the launch intent was forgotten the moment the first turn honored it, so anything that drifted afterwards went unannounced. That trade was made on the assumption that a later change can only be your own doing — true of the model picker, false of the effort one: a conversation started by a batch carries `CLAUDE_CODE_EFFORT_LEVEL`, which takes priority over `/effort` and cannot be overridden from inside the session (see Claude Code's model-config docs and issue anthropics/claude-code#39846). Its selector will happily show `max` while every single turn plays `medium`, silently. The panel now says so. The cost, stated plainly: after you deliberately switch model inside a batch conversation, the badge will remind you what the batch had asked for.
